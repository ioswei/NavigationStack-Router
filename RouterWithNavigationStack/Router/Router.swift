...
extension Router {
    ...
    func navigateBack(_ viewSpec: ViewSpec) {
        guard let listIndex = state.navigationPath.firstIndex(of: viewSpec) else {
            return
        }
        var nPath = Array(state.navigationPath.prefix(through: listIndex))
        self.replaceNavigationStack(path: nPath)
    }
